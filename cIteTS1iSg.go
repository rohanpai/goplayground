...
func main() {
	docs := []string{testXML,testXML2}
	for _,v := range docs {
		doc, _ := DocToMap(v)
		fmt.Println(ValuesForKey(doc,"disable")[0].(string))
		fmt.Println(ValuesForKey(doc,"text1")[0].(string))
		fmt.Println(ValuesForKey(doc,"word1")[0].(string))
	}
}
